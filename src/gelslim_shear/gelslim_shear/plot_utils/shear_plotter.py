...
def cv_plot_vector_field(vf, ch_dim=0,
                         image_size=(600, 600),
                         color=(0, 255, 0),
                         thickness=2,
                         tip_length=0.3,
    ...
    """
    vf: numpy array of shape (2, H, W) if ch_dim=0
    ...
    # white canvas
    img = 255*np.ones((int(round(H)), int(round(W)), 3), dtype=np.uint8)

    # draw every arrow at its cell-center
    for i in range(rows):
        for j in range(cols):
            ...
            u_val = float(u[i, j])
            v_val = float(v[i, j])
            if not (math.isfinite(u_val) and math.isfinite(v_val)):
                continue

            dx = int(round(u_val * scale))
            dy = int(round(v_val * scale))
            # if your +y is “up” in vf, do: dy = -dy
...
def cv_plot_scalar_field(sf, max_magnitude=1.0, colormap=cv2.COLORMAP_JET):
    """
    sf: 2-D numpy array of shape (H, W).
    Returns a color-mapped image of shape (H, W, 3).
    """
    if isinstance(sf, torch.Tensor):
        sf = sf.cpu().numpy()
    clipped = np.clip(sf, -max_magnitude, max_magnitude)
    normed  = ((clipped + max_magnitude) / (2*max_magnitude) * 255).astype(np.uint8)

    img = cv2.applyColorMap(normed, colormap)
    return img
