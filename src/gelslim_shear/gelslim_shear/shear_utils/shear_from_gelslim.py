...
class ShearGenerator():
    ...
    def update_shear(self):
        image = self.get_grayscale(self.tactile_image.permute(1,2,0).cpu().numpy().astype(np.uint8))
        if self.prev_tactile_image is None:
            self.prev_tactile_image = self.tactile_image
        prev_shear_image = self.get_grayscale(self.prev_tactile_image.permute(1,2,0).cpu().numpy().astype(np.uint8))
        no_shear_image = self.get_grayscale(self.base_tactile_image.permute(1,2,0).cpu().numpy().astype(np.uint8))
        if self.method == 'weighted' or self.method == '1':
